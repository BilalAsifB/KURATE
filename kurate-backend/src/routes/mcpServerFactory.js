...
import { z } from "zod";
import {
  listPrompts, getLatestPromptVersion, getPromptVersion, listPromptVersions,
} from "../database/repository.js";

export function parseNamespacedRef(ref) {
  const match = ref.match(/^(.+)-v(\d+)$/);
  if (match) return { namespace: match[1], version: Number(match[2]) };
...
export function createKurateMcpServer() {
  ...
  server.registerResource(
    ...
    { title: "Kurate Compiled Context", mimeType: "text/markdown" },
    async (uri, { namespacedRef }) => {
      ...
      if (!record) throw new Error(`No prompt found for '${uri.href}'.`);
      return { contents: [{ uri: uri.href, mimeType: "text/markdown", text: record.compiled_prompt }] };
    }
  );
  ...
  server.registerTool(
    "kurate_get_context",
    {
      ...
      description: "Fetch a compiled context from the Kurate registry by namespace and optional version.",
      inputSchema: {
        namespace: z.string().describe("The prompt namespace, e.g. 'api-docs'"),
        version: z.number().int().positive().optional().describe("Specific version number. Omit for latest."),
      },
    },
    async ({ namespace, version }) => {
      ...
      if (!record) {
        return {
          isError: true,
          content: [{ type: "text", text: `No prompt found for namespace '${namespace}'${version ? ` at version ${version}` : ""}.` }],
        };
      }
  ...
  server.registerTool(
    ...
    async () => {
      ...
      return { content: [{ type: "text", text: summary.length ? summary.join("\n") : "No saved contexts yet." }] };
    }
  );
