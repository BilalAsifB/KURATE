...
export async function markDocumentReady({ documentId, markdown, assetCount }) {
  const { rows } = await query(
    `UPDATE documents
     SET status = 'ready', markdown = $2, asset_count = $3
     WHERE id = $1
     RETURNING *`,
...
export async function deleteDocument(documentId) {
  const { rowCount } = await query(`DELETE FROM documents WHERE id = $1`, [
    documentId,
  ]);
  return rowCount > 0;
}
...
export async function getPromptByNamespace(namespace) {
  const { rows } = await query(`SELECT * FROM prompts WHERE namespace = $1`, [
    namespace,
  ]);
  return rows[0] || null;
}
  ...
  return withTransaction(async (client) => {
    ...
    const versionResult = await client.query(
      `INSERT INTO prompt_versions
         (prompt_id, version, instructions, cart_items, compiled_prompt)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`,
      [
        prompt.id,
    ...
    return {
      prompt: { ...prompt, latest_version: nextVersion },
